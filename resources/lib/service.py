...
class SetAddonConstants:
	def run(self):
		kodi_utils.logger('CatLight', 'SetAddonConstants Service Starting')
		import random
		addon_items = [
			('catlight.playback_key', str(random.randint(1000, 10000))),
			('catlight.addon_version', kodi_utils.addon_info('version')),
			...
			('catlight.addon_fanart', kodi_utils.addon_fanart())
					]
		for item in addon_items: kodi_utils.set_property(*item)
		return kodi_utils.logger('CatLight', 'SetAddonConstants Service Finished')

class DatabaseMaintenance:
	def run(self):
		kodi_utils.logger('CatLight', 'DatabaseMaintenance Service Starting')
		from caches.base_cache import check_databases_integrity
		check_databases_integrity(silent=True)
		return kodi_utils.logger('CatLight', 'DatabaseMaintenance Service Finished')

class SyncSettings:
	def run(self):
		kodi_utils.logger('CatLight', 'SyncSettings Service Starting')
		sync_settings()
		return kodi_utils.logger('CatLight', 'SyncSettings Service Finished')

class OnUpdateChanges:
	def run(self):
		kodi_utils.logger('CatLight', 'OnUpdateChanges Service Starting')
		try:
		...
		return kodi_utils.logger('CatLight', 'OnUpdateChanges Service Finished')
	...
	def refresh_addon_keys(self):
		...
		show_dialog = False
		if get_setting('catlight.tmdb_api') == 'b370b60447737762ca38457bd77579b3':
			...
			show_dialog = True
		if get_setting('catlight.trakt.client') == '1038ef327e86e7f6d39d80d2eb5479bff66dd8394e813c5e0e387af0f84d89fb':
			...
			show_dialog = True
		if show_dialog:
			text = 'The original developer of CatLight has revoked all keys (Trakt, TMDb) that have been used up until now '\
			'within this addon, with permission, due to security concerns.'
			kodi_utils.ok_dialog(heading='Addon Credentials Reset', text=text)
			text = 'This unfortunately means you will need to re-authenticate your Trakt & TMDb accounts through CatLight AM  if you are currently using them. ' \
			'My apologies, but with the previous keys being revoked, this is necessary.'
			kodi_utils.ok_dialog(heading='Addon Credentials Reset', text=text)

class CustomWindowsPrepare:
	def run(self):
		kodi_utils.logger('CatLight', 'CustomWindowsPrepare Service Starting')
		...
		return kodi_utils.logger('CatLight', 'CustomWindowsPrepare Service Finished')

class TraktMonitor:
	def run(self):
		kodi_utils.logger('CatLight', 'TraktMonitor Service Starting')
		...
		while not monitor.abortRequested():
			...
			try:
				...
				if status == 'failed': kodi_utils.logger('CatLight', trakt_service_string % ('Failed. Error from Trakt', next_update_string))
				elif status == 'no_auth': kodi_utils.logger('CatLight', trakt_service_string % ('Not Run. No Current Trakt Account', next_update_string))
				else:
					if status in ('success', 'no account'):
						kodi_utils.logger('CatLight', trakt_service_string % ('Success. %s' % trakt_success_line_dict[status], next_update_string))
					else:
						kodi_utils.logger('CatLight', trakt_service_string % ('Success. No Changes Needed', next_update_string))# 'not needed'
					if status == 'success' and get_setting('catlight.trakt.refresh_widgets', 'false') == 'true':
						kodi_utils.run_plugin({'mode': 'kodi_refresh'})
			except Exception as e: kodi_utils.logger('CatLight', trakt_service_string % ('Failed', 'The following Error Occured: %s' % str(e)))
			wait_for_abort(wait_time)
		try: del monitor
		except: pass
		try: del player
		except: pass
		return kodi_utils.logger('CatLight', 'TraktMonitor Service Finished')

class UpdateCheck:
	def run(self):
		if kodi_utils.get_property(firstrun_update_prop) == 'true': return
		kodi_utils.logger('CatLight', 'UpdateCheck Service Starting')
		...
		return kodi_utils.logger('CatLight', 'UpdateCheck Service Finished')

class WidgetRefresher:
	def run(self):
		kodi_utils.logger('CatLight', 'WidgetRefresher Service Starting')
		from time import time
		monitor, player = kodi_utils.kodi_monitor(), kodi_utils.kodi_player()
		...
		while not monitor.abortRequested():
			try:
				...
				if self.next_refresh < time():
					kodi_utils.logger('CatLight', 'WidgetRefresher Service - Widgets Refreshed')
		...
		return kodi_utils.logger('CatLight', 'WidgetRefresher Service Finished')
...
class AutoStart:
	def run(self):
		kodi_utils.logger('CatLight', 'AutoStart Service Starting')
		from modules.settings import auto_start_catlight
		if auto_start_catlight(): kodi_utils.run_addon()
		return kodi_utils.logger('CatLight', 'AutoStart Service Finished')

class AddonXMLCheck:
	def run(self):
		kodi_utils.logger('CatLight', 'AddonXMLCheck Service Starting')
		...
		return kodi_utils.logger('CatLight', 'AddonXMLCheck Service Finished')
	...
	def change_xml_file(self):
		...
		kodi_utils.logger('CatLight', 'AddonXMLCheck Service - Change Detected. Restarting Addons')
	...
	def reassign_addon_icon(self):
		...
		addon_icon_choice({'set_icon': get_setting('addon_icon_choice_name', 'catlight_icon_01.png')})
...
class CatlightMonitor(Monitor):
	...
	def onNotification(self, sender, method, data):
		if method in ('GUI.OnScreensaverActivated', 'System.OnSleep'):
			...
			kodi_utils.logger('OnNotificationActions', 'PAUSING CatLight Services Due to Device Sleep')
		elif method in ('GUI.OnScreensaverDeactivated', 'System.OnWake'):
			kodi_utils.clear_property(pause_services_prop)
			kodi_utils.logger('OnNotificationActions', 'UNPAUSING CatLight Services Due to Device Awake')

kodi_utils.logger('CatLight', 'Main Monitor Service Starting')
CatlightMonitor().waitForAbort()
kodi_utils.logger('CatLight', 'Main Monitor Service Finished')
