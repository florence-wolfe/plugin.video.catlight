...
def get_icon(image_name, image_folder='icons', image_type='png'):
	return 'https://raw.githubusercontent.com/%s/%s/main/packages/media/%s/%s.%s' \
			% (get_property('catlight.update.username'), get_property('catlight.update.location'), image_folder, image_name, image_type)
...
def refresh_widgets():
	...
	kodi_refresh()
	if get_setting('catlight.widget_refresh_notification', 'true') == 'true': notification('Widgets Refreshed', 2500)
...
def notification(line1, time=5000, icon=None):
	kodi_dialog().notification('CatLight', line1, icon or addon_icon(), time)
...
def player_check(mode, params):
	...
	if mode == 'playback.%s' % playback_key():
		...
		CatlightPlayer().run(params.get('url', None), params.get('obj', None))
...
def get_all_icons():
	...
	username, location = get_property('catlight.update.username'), get_property('catlight.update.location')
