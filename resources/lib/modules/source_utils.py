...
def internal_results(provider, sources):
	set_property('catlight.internal_results.%s' % provider, json.dumps(sources))
...
def get_external_cache_status(debrid, unchecked_hashes, data, active_debrid):
	...
	try:
		results = []
		imdb_id = data['imdb']
		debrid_name, services, token = {'Real-Debrid': ('realdebrid', ['torrentio'], get_setting('catlight.rd.token')),
										'AllDebrid': ('alldebrid', ['mediafusion'], get_setting('catlight.ad.token'))}[debrid]
