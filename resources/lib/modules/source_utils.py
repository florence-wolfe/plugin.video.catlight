...
def internal_results(provider, sources):
	set_property('flolight.internal_results.%s' % provider, json.dumps(sources))
...
def get_external_cache_status(debrid, unchecked_hashes, data, active_debrid):
	...
	try:
		results = []
		imdb_id = data['imdb']
		debrid_name, services, token = {'Real-Debrid': ('realdebrid', ['torrentio'], get_setting('flolight.rd.token')),
										'AllDebrid': ('alldebrid', ['mediafusion'], get_setting('flolight.ad.token'))}[debrid]
