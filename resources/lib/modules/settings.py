...
def ai_model_active():
	if get_setting('catlight.google_api', 'empty_setting') not in (None, 'None', '', 'empty_setting'): return True
	if get_setting('catlight.groq_api', 'empty_setting') not in (None, 'None', '', 'empty_setting'): return True
	return False
...
def autoscrape_confirm():
	return get_setting('catlight.autoscrape_confirm', 'false') == 'true'
...
def auto_nextep_settings(play_type):
	...
	watching_check = int(get_setting('catlight.autoplay_watching_check', '3'))
...
def tmdblists_sort_order(setting):
	...
	return str(get_setting('catlight.tmdbsort.%s' % setting, '4'))
...
def easynews_playback_method_retries():
	return int(get_setting('catlight.easynews.playback_method_retries', '1')) + 1
...
def extras_enabled_ratings():
	return get_setting('catlight.extras.enabled_ratings', 'Meta, Tom/Critic, Tom/User, IMDb, TMDb').split(', ')

def extras_enable_item_ratings():
	return get_setting('catlight.extras.enable_item_ratings', 'false') =='true'

def extras_enable_scrollbars():
	return get_setting('catlight.extras.enable_scrollbars', 'false')
...
def extras_order():
	setting = get_setting('catlight.extras.order', '2000,2050,2051,2052,2053,2054,2055,2056,2057,2058,2059,2060,2061,2062')
	if setting in ('', None, 'noop', []): return []
	split_setting = setting.split(',')
	return [int(i) for i in split_setting]
...
def check_prescrape_sources(scraper, media_type):
	if scraper in ('easynews', 'rd_cloud', 'pm_cloud', 'ad_cloud', 'tb_cloud', 'folders'): return get_setting('catlight.check.%s' % scraper) == 'true'
...
def uncached_min_seeders():
	return int(get_setting('catlight.results.uncached_min_seeders', '0'))
...
def sort_to_top(provider):
	sort_to_top_dict = {'folders': 'catlight.results.sort_folders_first', 'rd_cloud': 'catlight.results.sort_rdcloud_first', 'pm_cloud': 'catlight.results.sort_pmcloud_first',
						'ad_cloud': 'catlight.results.sort_adcloud_first', 'tb_cloud': 'catlight.results.sort_tbcloud_first'}
...
def jump_to_enabled():
	return get_setting('catlight.paginate.jump_to', 'true') == 'true'
...
def rescrape_settings():
	...
	return sorted([(i[0], int(get_setting('catlight.rescrape.%s' % i[0], i[1])), int(get_setting('catlight.rescrape.%s.order' % i[0], i[2]))  ) \
					for i in rescrapes if int(get_setting('catlight.rescrape.%s' % i[0], i[1])) in (1, 2)], key=lambda x: x[2])
...
def cm_enabled():
	...
	setting = get_setting('catlight.context_menu.enabled', default)
...
def cm_current_order():
	default = 'extras,options,playback_options,browse_movie_set,browse_seasons,browse_episodes,recommended,related,more_like_this,similar,in_trakt_list,' \
				'trakt_manager,personal_manager,tmdb_manager,favorites_manager,mark_watched,unmark_previous_episode,exit,refresh,reload'
	setting = get_setting('catlight.context_menu.order', default)
	if setting in ('', None, 'noop', '[]'): return default.split(',')
	return setting.split(',')
...
def rpdb_info(media_type):
	...
	else: active = int(get_setting('catlight.rpdb_enabled', '0')) in {'movie': (1, 3), 'tvshow': (2, 3)}[media_type]
	if active: return {'rpdb_api_key': get_setting('catlight.rpdb_api'), 'rpdb_format': get_setting('catlight.rpdb_format')}
