...
class Movies:
    ...
    special = (
        ...
        "tmdb_movie_keyword_results_direct",
        "ai_similar",
    )
    personal = {
        "in_progress_movies": ("modules.watched_status", "get_in_progress_movies"),
        "favorites_movies": ("modules.favorites", "get_favorites"),
    ...
    def __init__(self, params):
        ...
        self.category_name = (
            self.params_get("category_name", None)
            or self.params_get("name", None)
            or "Movies"
        )
        self.id_type, self.list, self.action = (
            self.params_get("id_type", "tmdb_id"),
            self.params_get("list", []),
            self.params_get("action", None),
        )
        self.items, self.new_page, self.total_pages, self.is_external = (
            [],
            {},
            None,
            kodi_utils.external(),
        ...
        if self.is_external:
            ...
            self.widget_hide_watched = (
                self.action not in ("watched_movies", "recent_watched_movies")
                and settings.widget_hide_watched()
            )
        else:
            self.widget_hide_next_page, self.widget_hide_watched = False, False
    ...
    def fetch_list(self):
        ...
        try:
            ...
            if self.action in self.personal:
                ...
                var_module, import_function = (
                    "apis.%s_api" % self.action.split("_")[0],
                    self.action,
                )
            try:
            ...
            if page_no == 1 and not self.is_external:
                kodi_utils.set_property(
                    "catlight.exit_params", kodi_utils.folder_path()
                )
            if self.action in self.main:
                ...
                if data["total_pages"] > page_no:
                    self.new_page = {"new_page": str(data["page"] + 1)}
                ...
                self.list = [i["id"] for i in results]
                if data["total_pages"] > page_no:
                    self.new_page = {"url": url, "new_page": str(data["page"] + 1)}
            elif self.action == "tmdb_movies_sets":
                self.movieset_list_active = True
                data = sorted(
                    movieset_meta(self.params_get("key_id"), self.tmdb_api_key)[
                        "parts"
                    ],
                    key=lambda k: k["release_date"] or "2050",
                )
                ...
                if self.params_get("get_imdb"):
                    self.params["key_id"] = movie_meta(
                        "tmdb_id",
                        self.params_get("key_id"),
                        self.tmdb_api_key,
                        self.mpaa_region,
                        get_datetime(),
                        get_current_timestamp(),
            ...
            if self.new_page and not self.widget_hide_next_page:
                ...
                kodi_utils.add_dir(
                    handle,
                    self.new_page,
                    "Next Page (%s) >>" % self.new_page["new_page"],
                    "nextpage",
        ...
        kodi_utils.end_directory(
            handle, cacheToDisc=False if self.is_external else True
        )
    ...
    def build_movie_content(self, _position, _id):
        try:
            ...
            if self.rpdb_api_key:
                try:
                    poster = meta_get("rpdb_poster") % self.rpdb_api_key
                except:
                    poster = meta_get("poster") or self.poster_empty
            else:
                poster = meta_get("poster") or self.poster_empty
            fanart = meta_get("fanart") or self.fanart_empty
            clearlogo, landscape = (
                meta_get("clearlogo") or "",
                meta_get("landscape") or "",
            )
            thumb = poster or landscape or fanart
            movieset_id, movieset_name = (
                meta_get("extra_info").get("collection_id", None),
                meta_get("extra_info").get("collection_name", None),
            )
            first_airdate = jsondate_to_datetime(premiered, "%Y-%m-%d", True)
            duration = meta_get("duration")
            if not first_airdate or self.current_date < first_airdate:
                unaired = True
            else:
                unaired = False
            ...
            playcount = watched_status.get_watched_status_movie(
                self.watched_info, str_tmdb_id
            )
            play_params = self.build_url(
                {
                    "mode": "playback.meta",
                    "media_type": "movie",
                    "tmdb_id": tmdb_id,
                    self.playback_key: "true",
                }
            )
            extras_params = self.build_url(
                {
            ...
            playback_options_params = self.build_url(
                {"mode": "playback_choice", "media_type": "movie", "meta": tmdb_id}
            )
            browse_recommended_params = self.build_url(
                {
                    "mode": "build_movie_list",
                    "action": "tmdb_movies_recommendations",
                    "is_external": self.is_external,
                    "key_id": tmdb_id,
                    "name": "Recommended based on %s" % title,
                }
            )
            browse_more_like_this_params = self.build_url(
                {
                    "mode": "build_movie_list",
            ...
            browse_similar_params = self.build_url(
                {
                    "mode": "build_movie_list",
                    "action": "ai_similar",
                    "is_external": self.is_external,
                    "key_id": "movie|%s" % tmdb_id,
                    "name": "AI Similar based on %s" % title,
                }
            )
            trakt_manager_params = self.build_url(
                {
            ...
            personal_manager_params = self.build_url(
                {
                    "mode": "personallists_manager_choice",
                    "list_type": "movie",
                    "tmdb_id": tmdb_id,
                    "title": title,
                    "premiered": premiered,
            ...
            tmdb_manager_params = self.build_url(
                {
                    "mode": "tmdblists_manager_choice",
                    "media_type": "movie",
                    "tmdb_id": tmdb_id,
                    "icon": poster,
                }
            )
            favorites_manager_params = self.build_url(
                {
                    "mode": "favorites_manager_choice",
                    "media_type": "movie",
                    "tmdb_id": tmdb_id,
                    "title": title,
                }
            ...
            belongs_to_movieset = (
                "true" if all([movieset_id, movieset_name]) else "false"
            )
            movieset_active = self.open_movieset and belongs_to_movieset == "true"
            if self.open_extras or movieset_active:
                cm_append(
                    ["extras", ("[B]Playback[/B]", "RunPlugin(%s)" % play_params)]
                )
            if not self.open_extras or movieset_active:
                cm_append(
                    ["extras", ("[B]Extras[/B]", "RunPlugin(%s)" % extras_params)]
                )
            if movieset_active:
                url_params = self.build_url(
                    {
                        "mode": "open_movieset_choice",
                        "key_id": movieset_id,
                        "name": movieset_name,
                        "is_external": self.is_external,
                    }
                )
            elif self.open_extras:
            ...
            cm_append(
                [
                    ...
                    (
                        ...
                        "RunPlugin(%s)" % playback_options_params,
                    ),
                ]
            )
            if (
                belongs_to_movieset == "true"
                and not self.movieset_list_active
                and not self.open_movieset
            ):
                browse_movie_set_params = self.build_url(
                    {
                        "mode": "build_movie_list",
                ...
                cm_append(
                    [
                        "browse_movie_set",
                        (
                            "[B]Browse Movie Set[/B]",
                            self.window_command % browse_movie_set_params,
                        ),
                    ]
                )
            else:
                browse_movie_set_params = ""
            cm_append(
                [
                    "recommended",
                    (
                        "[B]Browse Recommended[/B]",
                        self.window_command % browse_recommended_params,
                    ),
                ]
            )
            cm_append(
                [
                    "more_like_this",
                    (
                        "[B]Browse More Like This[/B]",
                        self.window_command % browse_more_like_this_params,
                    ),
                ]
            )
            cm_append(
                [
                    "similar",
                    (
                        "[B]Browse AI Similar[/B]",
                        self.window_command % browse_similar_params,
                    ),
                ]
            )
            if imdb_id:
                browse_in_trakt_list_params = self.build_url(
                    {
                ...
                cm_append(
                    [
                        ...
                        (
                            "[B]In Trakt Lists[/B]",
                            self.window_command % browse_in_trakt_list_params,
                        ),
                    ]
                )
            else:
                browse_in_trakt_list_params = ""
            cm_append(
                [
                    "trakt_manager",
                    (
                        "[B]Trakt Lists Manager[/B]",
                        "RunPlugin(%s)" % trakt_manager_params,
                    ),
                ]
            )
            cm_append(
                [
                    "personal_manager",
                    (
                        "[B]Personal Lists Manager[/B]",
                        "RunPlugin(%s)" % personal_manager_params,
                    ),
                ]
            )
            cm_append(
                [
                    "tmdb_manager",
                    (
                        "[B]TMDb Lists Manager[/B]",
                        "RunPlugin(%s)" % tmdb_manager_params,
                    ),
                ]
            )
            cm_append(
                [
                    "favorites_manager",
                    (
                        "[B]Favorites Manager[/B]",
                        "RunPlugin(%s)" % favorites_manager_params,
                    ),
                ]
            )
            if playcount:
                if self.widget_hide_watched:
                    return
                cm_append(
                    [
                        "mark_watched",
                        (
                            "[B]Mark Unwatched %s[/B]" % self.watched_title,
                            "RunPlugin(%s)"
                            % self.build_url(
                                {
                ...
                cm_append(
                    [
                        "mark_watched",
                        (
                            "[B]Mark Watched %s[/B]" % self.watched_title,
                            "RunPlugin(%s)"
                            % self.build_url(
                                {
                                    "mode": "watched_status.mark_movie",
                ...
                cm_append(
                    [
                        ...
                        (
                            ...
                            % self.build_url(
                                {
                                    "mode": "watched_status.erase_bookmark",
                                    "media_type": "movie",
                                    "tmdb_id": tmdb_id,
                                    "refresh": "true",
                                }
                ...
                cm_append(
                    [
                        ...
                        (
                            "[B]Exit Movie List[/B]",
                            "RunPlugin(%s)"
                            % self.build_url({"mode": "navigator.exit_media_menu"}),
                        ),
                    ]
                )
            if self.is_external:
                    ...
                    [
                        [
                            ...
                            (
                                "[B]Refresh Widgets[/B]",
                                "RunPlugin(%s)"
                                % self.build_url({"mode": "refresh_widgets"}),
                            ),
                        ],
                        [
                            "reload",
                            (
                                "[B]Reload Widgets[/B]",
                                "RunPlugin(%s)"
                                % self.build_url({"mode": "kodi_refresh"}),
                            ),
                        ],
                    ]
                )
            ...
            if progress:
                info_tag.setResumePoint(
                    watched_status.get_resume_seconds(progress, duration)
                )
    ...
    def worker(self):
        self.kodi_actor, self.make_listitem, self.build_url = (
            kodi_utils.kodi_actor(),
            kodi_utils.make_listitem,
            kodi_utils.build_url,
        )
        self.poster_empty, self.fanart_empty = (
            kodi_utils.get_icon("box_office"),
            kodi_utils.addon_fanart(),
        )
        self.current_date, self.current_time, self.watched_indicators = (
            get_datetime(),
            get_current_timestamp(),
        ...
        self.watched_info, self.bookmarks = (
            watched_status.watched_info_movie(watched_db),
            watched_status.get_bookmarks_movie(watched_db),
        )
        self.window_command = (
        ...
        if self.custom_order:
            threads = TaskPool().tasks(
                ...
                self.list,
                min(len(self.list), settings.max_threads()),
            )
    ...
    def sort_context_menu(self, context_menu_items):
        if self.perform_cm_sort:
            try:
                context_menu_items = sorted(
                    [i for i in context_menu_items if i[0] in self.cm_sort_order],
                    key=lambda k: self.cm_sort_order[k[0]],
                )
            except:
                pass
