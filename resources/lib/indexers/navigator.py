...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'Cat Light')
	...
	def tools(self):
		...
		if get_setting('catlight.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('catlight.use_viewtypes', 'true') == 'true' and not get_setting('catlight.manual_viewtypes', 'false') == 'true':
	...
	def maintenance(self):
		...
		self.add({'mode': 'clear_navigator_cache', 'isFolder': 'false'}, 'Clear Navigator Cache', 'settings')
	...
	def changelog_utils(self):
		...
		catlight_clogpath = k.translate_path('special://home/addons/plugin.video.catlight/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': catlight_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.catlight' in iconImage: original_image = True
	...
	def exit_media_menu(self):
		params = k.get_property('catlight.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.catlight/resources/text/tips'
