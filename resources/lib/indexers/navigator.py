...
class Navigator:
	def __init__(self, params):
		...
		self.category_name = self.params_get('name', 'Flo Light')
	...
	def tools(self):
		...
		if get_setting('flolight.external_scraper.module') not in ('empty_setting', ''):
		...
		if get_setting('flolight.use_viewtypes', 'true') == 'true' and not get_setting('flolight.manual_viewtypes', 'false') == 'true':
	...
	def changelog_utils(self):
		...
		flolight_clogpath = k.translate_path('special://home/addons/plugin.video.flolight/resources/text/changelog.txt')
		self.add({'mode': 'show_text', 'heading': 'Changelog', 'file': flolight_clogpath, 'font_size': 'large', 'isFolder': 'false'}, 'Changelog', 'lists')
	...
	def build_shortcut_folder_contents(self):
		...
		if contents:
			for count, item in enumerate(contents):
				...
				icon = iconImage
				if iconImage:
					if iconImage.startswith('http') or 'plugin.video.flolight' in iconImage: original_image = True
	...
	def exit_media_menu(self):
		params = k.get_property('flolight.exit_params')
		if params: return k.container_refresh_input(params)

	def tips(self):
		tips_location = 'special://home/addons/plugin.video.flolight/resources/text/tips'
