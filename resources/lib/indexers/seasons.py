...
from modules.watched_status import (
    get_database,
    watched_info_season,
    get_watched_status_season,
    get_progress_status_season,
)
...
def build_season_list(params):
        ...
        for item in season_data:
            try:
                ...
                overview, poster_path, air_date = (
                    item_get("overview"),
                    item_get("poster_path"),
                    item_get("air_date"),
                )
                season_number, aired_eps = (
                    item_get("season_number"),
                    item_get("episode_count"),
                )
                ...
                try:
                    season_date, premiered = adjust_premiered_date(
                        air_date, adjust_hours
                    )
                except:
                    season_date, premiered = None, ""
                unaired = aired_eps == 0 or (season_date and current_date < season_date)
                if unaired or season_special:
                    progress, playcount, total_watched, total_unwatched = (
                        0,
                        0,
                        0,
                        aired_eps,
                    ...
                    if season_number < total_seasons:
                        ...
                        aired_eps = total_aired_eps - episode_count
                    playcount, watched, unwatched = get_watched_status_season(
                        watched_info.get(season_number, None), aired_eps
                    )
                ...
                if playcount:
                    ...
                    cm_append(
                        [
                            "mark_watched",
                            (
                                "[B]Mark Watched %s[/B]" % watched_title,
                                "RunPlugin(%s)"
                                % build_url(
                                    {
                                        "mode": "watched_status.mark_season",
                ...
                if progress:
                    cm_append(
                        [
                            "mark_watched",
                            (
                                "[B]Mark Unwatched %s[/B]" % watched_title,
                                "RunPlugin(%s)"
                                % build_url(
                                    {
                                        "mode": "watched_status.mark_season",
                        ...
                        [
                            [
                                ...
                                (
                                    "[B]Refresh Widgets[/B]",
                                    "RunPlugin(%s)"
                                    % build_url({"mode": "refresh_widgets"}),
                                ),
                            ],
                            [
                                "reload",
                ...
                if perform_cm_sort:
                    try:
                        cm = sorted(
                            [i for i in cm if i[0] in cm_sort_order],
                            key=lambda k: cm_sort_order[k[0]],
                        )
                    except:
                        pass
                cm = [i[1] for i in cm]
                ...
                listitem.setArt(
                    {
                        ...
                        "fanart": show_fanart,
                        "clearlogo": show_clearlogo,
                        "landscape": show_landscape,
                        "thumb": thumb,
                        "icon": show_landscape,
                        "tvshow.poster": poster,
                        "tvshow.clearlogo": show_clearlogo,
                    }
                )
    ...
    meta_get = meta.get
    tmdb_id, tvdb_id, imdb_id, show_title, show_year = (
        meta_get("tmdb_id"),
        meta_get("tvdb_id"),
        meta_get("imdb_id"),
        meta_get("title"),
        meta_get("year") or "2050",
    )
    orig_title, status, show_plot = (
        meta_get("original_title", ""),
        meta_get("status"),
        meta_get("plot"),
    )
    str_tmdb_id, str_tvdb_id, rating, genre = (
        str(tmdb_id),
        str(tvdb_id),
        meta_get("rating"),
        meta_get("genre"),
    ...
    mpaa, votes, trailer, studio, country = (
        meta_get("mpaa"),
        meta_get("votes"),
        str(meta_get("trailer")),
        meta_get("studio"),
        meta_get("country"),
    )
    episode_run_time, season_data, total_seasons = (
        meta_get("duration"),
        meta_get("season_data"),
        meta_get("total_seasons"),
    )
    rpdb_api_key = settings.rpdb_api_key("tvshow")
    if rpdb_api_key:
        try:
            show_poster = meta_get("rpdb_poster") % rpdb_api_key
        except:
            show_poster = meta_get("poster") or poster_empty
    else:
        show_poster = meta_get("poster") or poster_empty
    show_fanart = meta_get("fanart") or fanart_empty
    show_clearlogo, show_landscape = (
        meta_get("clearlogo") or "",
        meta_get("landscape") or "",
    )
    custom_order = params.get("custom_order", None)
    if settings.show_specials():
        season_data.sort(key=lambda i: (i["season_number"] == 0, i["season_number"]))
    elif custom_order is not None:
        season_data = [i for i in season_data if i["season_number"] == params["season"]]
    else:
    ...
    if custom_order is not None:
        return (list_items[0], custom_order)
...
def single_seasons(seasons_list):
    def _process(item):
        season_results_append(build_season_list(item))
