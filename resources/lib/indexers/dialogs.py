...
def window_theme_choice(params):
	if params['type'] == 'theme':
		...
		window_theme_opacity = get_setting('catlight.window_theme_opacity', 'CC')
		...
		window_theme = get_setting('catlight.window_theme', 'FF1F2020')[2:]
		window_theme_contrast = get_setting('catlight.window_theme_contrast', 'FF4a4347')
...
def addon_icon_choice(params):
	...
	from xml.dom.minidom import parse as mdParse
	large_image_url = 'https://raw.githubusercontent.com/CatlightAnonyMouse/CatlightAnonyMouse.github.io/main/packages/addon_icons/%s'
	...
	if set_icon: new_name = set_icon
		...
		try:
			...
			if not all_icons: kodi_utils.ok_dialog(heading='CatLight Icon Images', text='Error Fetching Icon Images')
		...
		except: return kodi_utils.ok_dialog(heading='CatLight Icon Images', text='Error Fetching Addon Icon Images') 
...
def extras_ratings_choice(params={}):
	...
	list_items = [{'line1': i[0], 'icon': 'catlight_flags/ratings/%s' % i[2]} for i in choices]
